...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y, Mc_x, Mc_y):
        ...
        self.Delta_C1 = 1.05 # cost intensive area 1 modifier
        self.Delta_C2 = 1.05 # cost intensive area 2 modifier
        self.Delta_C3 = 0.95 # cost intensive area 3 modifier
        self.costPerGrid = 1
...
def main():
    ...
    ox, oy = [], []
    for i in range(-20, 70): # draw the button border 
        ox.append(i)
        oy.append(-20.0)
    for i in range(-20, 70): # draw the right border
        ox.append(70.0)
        oy.append(i)
    for i in range(-20, 70): # draw the top border
        ox.append(i)
        oy.append(70.0)
    for i in range(-20, 70): # draw the left border
        ox.append(-20.0)
        oy.append(i)

    for i in range(-10, 20): # draw the free border (vertical line on x=20)
        ox.append(20.0)
        oy.append(i)

    for i in range(10, 30): # draw the diagonal obstacle line (from 10,40 to 30,20)
        ox.append(i)
        oy.append(-1 * i + 50)

    for i in range(40, 50): # draw the diagonal obstacle line (from 40,50 to 50,30)
        ox.append(i)
        oy.append(-2 * i + 130)
    ...
    if show_animation:  # pragma: no cover
        ...
        plt.plot(fc_x, fc_y, "oy", alpha=0.5) # plot the cost intensive area 1
        plt.plot(tc_x, tc_y, "or", alpha=0.5) # plot the cost intensive area 2
        plt.plot(Mc_x, Mc_y, "oc", alpha=0.1) # plot the cost reducing area 3

        plt.grid(True) # plot the grid to the plot panel
        plt.axis("equal") # set the same resolution for x and y axis 

    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y, Mc_x, Mc_y) # create the AStarPlanner class
